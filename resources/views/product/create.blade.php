                        ...
                        <div class="col ">
                            ...
                            <form action="{{ route('products.store') }}" method="post" enctype="multipart/form-data" id="product-form">
                                ...
                                <div id="sizes-container">
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div id="size-{{ strtoupper(str_replace(' ', '', $size->name)) }}" class="size-inputs">
                                            <input type="number" name="quantity{{ strtoupper(str_replace(' ', '', $size->name)) }}" value="{{ old('quantity' . strtoupper(str_replace(' ', '', $size->name)), 0) }}" min="0">
                                ...
                                <div>
                                    ...
                                    <textarea name="size_advice" id="size_advice" rows="3">{{ old('size_advice') }}</textarea>
                                    @error('size_advice')
                                    <span class="error-fdb">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div id="colors">
                                    ...
                                    <div class="d-flex flex-wrap">
                                        ...
                                        <label for="{{ $color->name }}" @class(['active'=> in_array($color->id, old('colors', []))]) @style(["background-color: $color->hex_code" , 'border: 0.17px solid #000000'=> strpos($color->hex_code, '#FFF') === 0])></label>
                                        <input type="checkbox" name="colors[]" id="{{ $color->name }}" value="{{ $color->id }}" {{ in_array($color->id, old('colors', [])) ? 'checked' : '' }}>
                                        @endforeach
                                    </div>
                                    @error('colors')
                                    <span class="error-fdb">{{ $message }}</span>
                                ...
                                <div>
                                    ...
                                    <textarea name="maintenance_guidelines" id="maintenance_guidelines" rows="3">{{ old('maintenance_guidelines') }}</textarea>
                                ...
                                <div>
                                    ...
                                    <select name="category" id="category">
                                        ...
                                        <option value="{{ $category->id }}" {{ $category->id == old('category') ? 'selected' : '' }}>{{ $category->display_name }}</option>
                                        @endforeach
                                    </select>
                                    @error('category')
                                    <span class="error-fdb">{{ $message }}</span>
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <select name="discount" id="discount">
                                                        ...
                                                        <option value="{{ $discount->id }}" {{ $discount->id == old('discount') ? 'selected' : '' }}>{{ $discount->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
